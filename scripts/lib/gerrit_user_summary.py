...
"""Console and GitHub step-summary reporting for Gerrit user setup.

Everything a human reads at the end of a user-setup run lives here:
the closing console block for a multi-instance loop, and the Markdown
appended to ``$GITHUB_STEP_SUMMARY`` for both the single-account and
multi-instance cases.

These writers are deliberately failure-tolerant.  A step summary that
cannot be written must never turn a successful account setup into a
failed action, so I/O errors are logged as warnings and swallowed.
"""
...
def log_loop_summary(username: str, success_count: int, failure_count: int) -> None:
    """Log the closing console block for a multi-instance setup run.

    Args:
        username: Account that was created or updated.
        success_count: Number of instances configured successfully.
        failure_count: Number of instances that failed.
    """
    logger.info("")
    logger.info("========================================")
    if failure_count == 0:
