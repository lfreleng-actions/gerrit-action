...
"""Structured reporting for replication-error scans.

Split out of :mod:`replication`; holds the per-line match record and
the report that separates the authoritative per-event log from the
advisory container log, classifies magic-repo and soft failures, and
renders the whole thing for the operator.  Both names are re-exported
from :mod:`replication`.
"""
