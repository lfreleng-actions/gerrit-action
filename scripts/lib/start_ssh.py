...
"""SSH material for a Gerrit instance.

Split out of ``start-instances.py``.  Holds the two SSH-facing steps:
staging the private key, ``known_hosts`` and ``ssh_config`` the
container needs to reach the source server, and capturing the host keys
the started container presents.  Both are re-exported from
``start-instances.py``.
"""
