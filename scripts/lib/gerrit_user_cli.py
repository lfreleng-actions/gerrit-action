...
"""Command-line interface definition for Gerrit user setup.

The user-setup entry point exposes four distinct groups of options —
connection, multi-instance looping, account details and SSH keys —
several of which default to environment variables set by the action.
Building the parser here keeps that surface (and its defaults, which
are part of the action's documented contract) separate from the
...
def build_parser(epilog: str | None = None) -> argparse.ArgumentParser:
    """Construct the ``setup-gerrit-user`` argument parser.

    Args:
        epilog: Long-form help text appended to ``--help`` output.
    ...
    _add_multi_instance_options(parser)
    _add_user_options(parser)
    _add_ssh_key_options(parser)
...
def _add_multi_instance_options(parser: argparse.ArgumentParser) -> None:
    """Add the options that drive the instances.json loop."""
...
def _add_ssh_key_options(parser: argparse.ArgumentParser) -> None:
    """Add the options supplying SSH public keys."""
