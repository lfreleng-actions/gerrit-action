...
from tunnel_probe import TunnelCheckResult, probe_with_urllib  # noqa: E402
from tunnel_report import (  # noqa: E402
    format_attempt_detail,
    log_tunnel_target,
    report_tunnel_failure,
    report_tunnel_success,
...
def probe_url(url: str, timeout: float = 10.0) -> TunnelCheckResult:
    ...
    return probe_with_urllib(url, timeout=timeout)
...
def run() -> int:
    ...
    log_tunnel_target(target)

    # --- Pre-flight diagnostics ---
    if debug:
        logger.debug("Running pre-flight network diagnostics…")
        for line in diagnose_host(target.host, target.port):
            logger.debug("  %s", line)
        logger.debug("")
    ...
    for attempt in range(1, target.max_attempts + 1):
        logger.info("  Attempt %d/%d: %s", attempt, target.max_attempts, target.url)

        result = probe_url(target.url, timeout=10.0)
        last_result = result

        if result.success:
            report_tunnel_success(target, result, attempt)
            return 0
        ...
        if attempt < target.max_attempts:
            logger.info("    Retrying in %ds…", target.retry_delay)
            time.sleep(target.retry_delay)

    # --- All attempts exhausted ---
    # ``diagnose_host`` is passed rather than called here so its DNS and
    # TCP round-trips stay sequenced *inside* the failure report, right
    # where their output appears.
    report_tunnel_failure(target, last_result, error_summary, diagnose_host)
